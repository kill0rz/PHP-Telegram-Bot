...
foreach ($triggersticker as $triggerword => $triggerstickerid) {
	if (isset($update["message"]["text"]) &&
		preg_match($triggerword, "", strtolower($update["message"]["text"]))) {
...
foreach ($triggersticker_perperson as $triggersticker) {
	if (isset($update["message"]["text"]) &&
		preg_match($triggersticker['searchpattern'], "", strtolower($update["message"]["text"])) &&
		$update["message"]["from"]["id"] == $triggersticker['userid']) {
